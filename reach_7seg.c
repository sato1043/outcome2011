...
/**
 * construct_reach_7seg
 *  - the function must precede others.
 *
 * @param thr porinter to reach_7seg_t
 */
...
/**
 * shutdown_reach_7seg
 *  - the function set terminate flag for reach_7seg thread.
 *
 * @param thr porinter to reach_7seg_t
 */
 ...
 *  - thread function.
 */
void reach_7seg(void *thr_)
{
  assert(thr_);
  reach_7seg_t* thr =(reach_7seg_t*)thr_;
  ...
  int n_ra ,n_rb;
  if (thr->pstate->preels->not_fixed(0))
  {
    n_ra =1;
    n_rb =2;
  }
  ...
  {
    n_ra =0;
    n_rb =(thr->pstate->preels->was_fixed(1))
      ? 1 : 2;
  }
  ...
  thr->pstate->rm_ctx(reach_7seg,thr_);
}
